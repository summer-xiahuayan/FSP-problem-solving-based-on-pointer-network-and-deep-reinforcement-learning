...
def reward(action_idxs, USE_CUDA=False):
    batch_size = len(action_idxs[0])
    sourceL=len(action_idxs)
    ...
    for i in range(batch_size):
        ...
        for j in range(sourceL):
            action_idx.append(action_idxs[j][i])
        s = Scheduling(Job, Machine, State, PT,agv_trans,agv_num)
        s.Decode(action_idx)
        processing_times[i]=s.fitness
...
def create_dataset(
    ...
    train_task = 'hfsp-size-{}-len-{}-train.txt'.format(train_size, data_len)
    val_task = 'hfsp-size-{}-len-{}-val.txt'.format(val_size, data_len)
...
class Scheduling:
    ...
    def api_return(self):
        ...
        return machinelists,agvlists,block_rate,agv_block_rates,self.fitness
...
class HfspDataset(Dataset):

    def __init__(self, dataset_fname):
        super(HfspDataset, self).__init__()
        ...
        with open(dataset_fname, 'r') as dset:
            ...
            for next_line in tqdm(lines):
                ...
                items=sum(Machine)
                sample = torch.zeros(items,len(toks)).long()

                #stage=0
                for idx in range(1,items+1):

                    for i in range(len(Machine)):
                        if idx<=sum(Machine[0:i+1]):
                            stage=i
                            break
                    print(stage)
                    ...
                    for idy, tok in enumerate(toks):
                        stage_time=PT[stage]
                        meachine_time=stage_time[idx-sum(Machine[0:stage])-1]
                        time=meachine_time[int(tok)]
                        sample[idx-1, idy] = int(time)
